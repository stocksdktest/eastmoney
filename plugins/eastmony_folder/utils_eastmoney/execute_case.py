import logging
from utils_eastmoney.yaml_utils import open_yaml
from utils_eastmoney.appium_proxy import appium_driver_proxy as driver_proxy
from utils_eastmoney.extract import * #extract_sh_sz, extract_sci_tech,extract_hk,extract_fund
...
class Execute_case():
    ...
    def exece_case(self, **kwargs):
        ...
        for i in range(len(data)-1):
            ...
            #     case_driver
                ...
                if data[i]['operate_type'] == 'click':
                    if data[i]['element_info'] == 'android.view.ViewGroup':
                        if kwargs['stock_type'] == 'sh_sz_index':
                            f = case_driver.find_element(key='text', value='指数')
                    else:
                        pass
                    f.click()
        ...
        elif data[-1]['operate_type'] == 'extract_QuoteDetailRequest_pankou':
            result = extract_QuoteDetailRequest_pankou(case_driver, **kwargs)

        elif data[-1]['operate_type'] == 'extract_TickRequest':
            result = extract_TickRequest(case_driver, **kwargs)
        elif data[-1]['operate_type'] == 'extract_QuoteDetailRequest':
            result = extract_QuoteDetailRequest_all(case_driver, **kwargs)
        else:
