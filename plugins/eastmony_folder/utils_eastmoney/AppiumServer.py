...
class AppiumServer(object):
    ...
    def start_server(self):
        for i in range(0, len(self.kwargs)):
            ...
            port = str(random.randint(4700, 4750))
            bp = str(random.randint(4751, 4800))
            port_bp = []
            flag = True
            while(flag):
                cmd = "appium  --session-override -p %s -bp %s -U %s" % (port, bp, self.kwargs[i]["devices"])
                ...
                while(True):
                    ...
                    if "listener started" in appium_result or "Error:listen" in appium_result :
                        print("-------server启动成功: %s---------" % port)
                        flag = False
                        port_bp.append((port, bp))
                        break
                    if "Fatal Error" in appium_result:
                        print("端口冲突")
                        break
                    if cnt >= 30:
                        break
